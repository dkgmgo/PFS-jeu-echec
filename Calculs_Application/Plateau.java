...
public class Plateau {
	...
	// test
	public Plateau() {
		pieces[2][2] = new Fou(true, new int[] {2,2});
	...
	/*public Plateau() {//tester grande boucle
		...
		for (int i = 0; i < 2; i++) {
			...
			pieces[0][a] = new Cavalier(b, new int [] {0,a});
			pieces[7][a] = new Cavalier(b, new int [] {7,a});
			pieces[1][a] = new Fou(b, new int [] {1, a});
			pieces[6][a] = new Fou(b, new int [] {6, a});
			pieces[2][a] = new Tour(b, new int[] {2, a});
			pieces[5][a] = new Tour(b, new int[] {5, a});
		}
		caseSelectionnee[0] = -1;
		caseSelectionnee[1] = -1;
	}*/
	...
	public void clicCase(int abs, int ord) {
		if (caseSelectionnee[0] == -1 && caseSelectionnee[1] == -1 && pieces[abs / 60][ord / 60] != null) { // selection
			...
			destinationsValides = pieces[ligne][colonne].deplacement(this);
		} else if (pieces[abs / 60][ord / 60] == null) { // deplacement simple
			caseSelectionnee = quelleCase(abs, ord);
			if (destinationsValides.contains(caseSelectionnee)) {
				pieces[ligne][colonne].setPlacement(caseSelectionnee);
