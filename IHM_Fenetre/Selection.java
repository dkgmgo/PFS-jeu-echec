...
public class Selection extends JFrame implements ActionListener {
	...
	private JScrollPane classement;
	private String texteClassement;
	private JTextArea areaClassement;
	...
	public Selection() {
		...
		cestParti.setBackground(new Color(255, 183, 152));
		cestParti.addActionListener(this);

		joueur1.setBounds(280, 250, 110, 50);
		joueur1.setBackground(new Color(154, 255, 255));
		joueur1.addActionListener(this);

		joueur2.setBounds(420, 250, 110, 50);
		joueur2.setBackground(new Color(154, 255, 255));
		...
		gameWithAI = new JButton("Jouer Contre L'ordinateur");
		gameWithAI.setBounds(350, 520, 200, 40);
		gameWithAI.setBackground(new Color(255, 183, 152));
		gameWithAI.setVisible(false);
		gameWithAI.addActionListener(this);
		...
		//classement
		texteClassement = connection.classement();
		areaClassement = new JTextArea(30,20);
		areaClassement.setText(texteClassement);
		classement = new JScrollPane(areaClassement);
	...
	public void actionPerformed(ActionEvent e) {
		...
		// interaction avec le bouton cestParti pour lancer le menu1 pour le joueur 2
		// puis ensuite lancer le menu2
		// on a initialise un boolean b2 qui ne prend la valeur true que lorsque le
		// menu1 pour le joueur 2 est ouvert
		// a ce moment la lorsqu'on râˆšÂ©appuiera sur le bouton cestParti, c'est le
		// menu2 qui s'ouvrira

		if (e.getSource() == cestParti && b2 == false) {

			// on stocke les noms des joueurs dans les attributs nom1 et nom2, pour pouvoir
		...
		// stockage de la donnÃˆe temps dans une variable si elle est donnÃˆe par le
		// joueur. Sinon, temps = 60 min
		...
		if (e.getSource() == lancerPartie) {
			j = new Jeu(nom1, mdp1, nom2, mdp2, temps * 30, commencer, connection);
			menu2.setVisible(false);
			j.setVisible(true);
		}
		...
		if (e.getSource() == check) {
			if (rappelSelection) {
				menu2.setVisible(true);
				texteClassement = connection.classement(); //on MAJ le classement
				areaClassement.setText(texteClassement);
			}
		}
	}
